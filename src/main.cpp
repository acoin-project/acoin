...
// Copyright (c) 2013-2014 Acoin developers
...
#if defined(NDEBUG)
# error "Acoin cannot be compiled without assertions."
#endif
...
static const int64_t nMinSubsidy = 0.01 * COIN;
...
int64_t GetBlockValue(int nHeight, int64_t nFees, uint256 prevHash)
{
    ...
    if (rand == 1)
    {
        cseed_str = prevHash.ToString().substr(14,7);
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        int randRewardFactor = generateMTRandom(seed, 10) + 1;
        nSubsidy *= randRewardFactor;
    }
...
static const int64_t nTargetTimespan = 40; // 40 seconds
static const int64_t nTargetSpacing = 40; // 40 seconds
static const int64_t nInterval = nTargetTimespan / nTargetSpacing; // 1 block

static const int64_t nAveragingInterval = nInterval * 10; // 10 blocks
static const int64_t nAveragingTargetTimespan = nAveragingInterval * nTargetSpacing; // 6 min 40 sec
